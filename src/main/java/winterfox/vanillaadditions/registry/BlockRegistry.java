...
public class BlockRegistry {
	...
	public static final RegistryObject<Block> ONLYFENCE_OAK = BLOCKS.register("onlyfence_oak", 
			() -> new OnlyFenceBlock(Block.Properties.of(Material.WOOD).strength(2.0F, 3.0F).sound(SoundType.WOOD)));
	...
	public static void onRegisterItems(final RegisterEvent event) {
		if(event.getRegistryKey().equals(ForgeRegistries.Keys.ITEMS)) {
			BLOCKS.getEntries().forEach((blockRegistryObject) -> {
				...
				Item.Properties properties = new Item.Properties().tab(ItemRegistry.VanillaAdditionsCreativeTab.instance);
